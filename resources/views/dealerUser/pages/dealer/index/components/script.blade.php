...
<script>
    ...
    function edit() {
        ...
        $.ajax({
            type: 'get',
            url: '{{ route('api.v1.dealerUser.dealer.show') }}',
            headers: {
    ...
    function show() {
        $(location).prop('href', `{{ route('dealerUser.dealer.show') }}/${$('#encrypted_id_edit').val()}/index`)
    }

    function save(method, data) {
        $.ajax({
            type: method,
            url: '{{ route('api.v1.dealerUser.dealer.save') }}',
            headers: {
                _token: '{{ auth()->user()->apiToken() }}',
                _auth_type: 'User'
            },
    ...
    var dealers = $('#dealers').DataTable({
        ...
        ajax: {
            type: 'get',
            url: '{{ route('api.v1.dealerUser.dealer.datatable') }}',
            headers: {
                _token: '{{ auth()->user()->apiToken() }}',
                _auth_type: 'User'
            },
    ...
    CreateButton.click(function () {
        var top_id = '{{ auth()->user()->getDealerId() }}';
    ...
    DeleteButton.click(function () {
        ...
        $.ajax({
            type: 'delete',
            url: '{{ route('api.v1.dealerUser.dealer.drop') }}',
            headers: {
                _token: '{{ auth()->user()->apiToken() }}',
                _auth_type: 'User'
            },
