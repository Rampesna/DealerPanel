...
<script>
    ...
    function show() {
        $(location).prop('href', `{{ route('customer.supportRequest.show') }}/${$('#encrypted_id_edit').val()}`);
    }

    function save(method, data) {
        $.ajax({
            type: method,
            url: '{{ route('api.v1.customer.supportRequest.save') }}',
            headers: {
    ...
    var supportRequests = $('#supportRequests').DataTable({
        ...
        ajax: {
            type: 'get',
            url: '{{ route('api.v1.customer.supportRequest.datatable') }}',
            headers: {
                _token: '{{ auth()->user()->apiToken() }}',
                _auth_type: 'Customer'
            },
